...
4. **Cross-legacy social-relationship guard (latent bug, surfaced during Task 10 review).** `addSocialRelationship` (now `lib/sims/social.ts`) has no same-legacy check, unlike `addFamilyRelationship`. A user owning two legacies can create a social edge across them; partner-adoption then recomputes only `simA.legacyId`. Pre-existing (not introduced by the refactor) and same-user-only (not a cross-tenant vuln), so it was left untouched to keep Task 10 behavior-preserving. Fix = add `if (simA.legacyId !== simB.legacyId) throw BAD_REQUEST 'Sims must belong to the same legacy'`, with a test, as its own change.
