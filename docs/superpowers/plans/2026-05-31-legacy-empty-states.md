...
1. **Inert CTAs.** "Name an heir →" (Succession) and "Record a moment →" (Milestones) are rendered as buttons with no action, per your instruction. When you later add real flows (e.g. a "name an heir" affordance on a sim, or — if ever — manual milestone entry), wire these up. If you'd prefer them to *link to the add-Sim page* in the meantime (like the Roster CTA), that's a one-line change per component.
